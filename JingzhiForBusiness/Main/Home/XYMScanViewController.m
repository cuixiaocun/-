...
- (void)viewDidLoad {
    ...
    {
        XYMScanView *scanV = [[XYMScanView alloc]initWithFrame:CGRectMake(0, Frame_NavAndStatus, ScreenWidth, ScreenHeight)];
        scanV.delegate = self;
        [self.view addSubview:scanV];
        _scanV = scanV;
...
- (void)getResult:(NSString *)scanDataString
{
    ...
    [PublicMethod AFNetworkPOSTurl:@"Home/member/fandsecurity" paraments:dic1  addView:self.view addNavgationController:self.navigationController  success:^(id responseDic) {
