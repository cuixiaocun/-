...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        xianOfGoods.frame =CGRectMake(0*Width,self.bottom-1.5*Width,CXCWidth,1.5*Width);
        xianOfGoods.tag =99;
...
-(void)setDic:(NSDictionary *)Dict
{
    ...
    timeLabel.frame =CGRectMake(24*Width ,cirLabel.bottom+10*Width, 600*Width,50*Width);
    timeLabel.text =[_dic objectForKey:@"time"];
    NSLog(@"%f",cirLabel.frame.size.height);
    UIImageView *imgvx=[self viewWithTag:99];
    imgvx.bottom =titleSize.height-1.5*Width+190*Width;
