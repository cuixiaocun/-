...
@implementation RegisterAndGoods
...
- (void)mainView
{
   ...
   //顶部提示
    ...
    UIView *bgview =[[UIView alloc]initWithFrame:CGRectMake(0,promptLabel.bottom, CXCWidth, 290*Width)];
    [bgScrollView addSubview:bgview];
    [bgview setBackgroundColor:[UIColor whiteColor]];
    //上部分代理级别及金额
    NSArray*topArr =@[@"代理级别",@"金额(¥)"];
    
    for (int i=0; i<2; i++) {
        //提示部分
        UILabel *promLabel =[[UILabel alloc]initWithFrame:CGRectMake(0+i*375*Width, 35*Width, 375*Width, 60*Width)];
        [promLabel setFont:[UIFont systemFontOfSize:14]];
        [bgview addSubview:promLabel];
        promLabel.textAlignment =NSTextAlignmentCenter;
        promLabel.text =topArr[i];
        [promLabel setTextColor:TextGrayColor];
        //代理级别
        if (i==0) {
            
            UILabel *displayLabel =[[UILabel alloc]initWithFrame:CGRectMake(0+i*375*Width, promLabel.bottom, 375*Width, 70*Width)];
        ...
        {//输入金额
            ...
            [inputText setPlaceholder:@"请输入交易金额"];
            ...
            [inputText setFrame:CGRectMake(0+i*375*Width, promLabel.bottom, 375*Width, 70*Width)];
    ...
    xian.frame =CGRectMake(374.5*Width,60*Width, 1*Width, 90*Width);
    //横线
