...
- (id)initWithFrame:(CGRect)frame cycleDirection:(CycleDirection)direction pictures:(NSArray *)pictureArray titleArray:(NSMutableArray *)titleArray
{
    ...
    if(self)
    {
       ...
       scrollView.pagingEnabled = YES;
        ...
        [imageview setFrame:CGRectMake(0, frame.size.height-40, frame.size.width, 40)];
        ...
        _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, frame.size.height-25, 320, 25)];
...
//        UILabel *titleLabel =[[UILabel alloc] initWithFrame:CGRectMake(0, frame.size.height-40, 320, 25)];
...
- (void)refreshScrollView {
    ...
    for (int i = 0; i < 3; i++) {
        EGOImageView *imageView = [[EGOImageView alloc] initWithPlaceholderImage:[UIImage imageNamed:@"picLoading2_1.png"]];
        [imageView setFrame:CGRectMake(0, 0, scrollFrame.size.width, scrollFrame.size.height)];
        imageView.userInteractionEnabled = YES;
        NSLog(@"%@",[[NSString stringWithFormat:@"%@",[curImages objectAtIndex:i] ] substringToIndex:4]);
        
        
        if ([[[NSString stringWithFormat:@"%@",[curImages objectAtIndex:i] ] substringToIndex:4] isEqualToString:@"http"]) {
            imageView.imageURL = [NSURL URLWithString: [curImages objectAtIndex:i]];

        }else
        {
            [imageView setImage:[UIImage imageNamed:[curImages objectAtIndex:i]]];
