...
@implementation ToubiaoDetailVC
...
- (void)mainView
{
    ...
    NSArray *btnArr =@[@"投标需求",@"竞标公司"];
    for (int i=0; i<2; i++) {
        UIButton *  statuBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        statuBtn.frame = CGRectMake(CXCWidth/2*i, 0,CXCWidth/2-2*Width ,85*Width);
        if (i==0) {
    ...
    [imageview setFrame:CGRectMake(0, Frame_NavAndStatus+85*Width, CXCWidth/2, 3*Width)];
...
- (void)changeStatuBtn:(UIButton *)btn
{
    ...
    [UIView animateWithDuration:0.15 animations:^{
        [imageview setFrame:CGRectMake(CXCWidth/2*(btn.tag-220), Frame_NavAndStatus+85*Width, CXCWidth/2, 3*Width)];
...
{
    if ([typeStr isEqualToString:@"220"]) {
        return  18*80*Width+2*20*Width+100*Width;
...
- (UITableViewCell *)tableView:(UITableView *)tableView1 cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([typeStr isEqualToString:@"220"])
    {
        ...
        ToubiaoDetailXQCell *cell =[tableView1 dequeueReusableCellWithIdentifier:CellIdentifier];
        if (cell == nil) {
            cell = [[ToubiaoDetailXQCell alloc] initWithStyle:UITableViewCellStyleDefault
    ...
    }else
    {
        ...
        ToubiaoDetailGSCell *cell =[tableView1 dequeueReusableCellWithIdentifier:CellIdentifier];
        if (cell == nil) {
            cell = [[ToubiaoDetailGSCell alloc] initWithStyle:UITableViewCellStyleDefault
                                               reuseIdentifier:CellIdentifier ];
            [cell setSelectionStyle:UITableViewCellSelectionStyleNone];
