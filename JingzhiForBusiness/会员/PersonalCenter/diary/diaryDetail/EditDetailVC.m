...
#import "AKGallery.h"
@interface EditDetailVC ()<YBImgPickerViewControllerDelegate>
{
...
{    bgScrollView =[[TPKeyboardAvoidingScrollView alloc] initWithFrame:CGRectMake(0, Frame_NavAndStatus+20*Width, CXCWidth, CXCHeight-Frame_NavAndStatus)];
    ...
    for (int i=0; i<3; i++) {
        ...
        }else if(i==1)
        {
            ...
            if([rightLabel.text isEqualToString:@"选择阶段"])
            {
        ...
        }else
        {
            ...
            self.contentTextView.text=NSLocalizedString(@"描述一下日记文章内容", nil);//提示语
