...
@interface ShopTalkListVC ()

@end

@implementation ShopTalkListVC
- (void)viewDidLoad {
    [super viewDidLoad];
...
{
    NSString *ideaContent =[infoArray[indexPath.row] objectForKey:@"content"];
    ...
    return 170*Width+ideaSize.height;
}
- (UITableViewCell *)tableView:(UITableView *)tableView1 cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    NSInteger row =[indexPath row];
    static NSString *CellIdentifier = @"Cell";
    ShopTalkListCell*cell =[tableView1 dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell == nil) {
        cell = [[ShopTalkListCell alloc] initWithStyle:UITableViewCellStyleDefault
...
- (void)getInfoList
{
    ...
    [PublicMethod AFNetworkPOSTurl:@"mobileapi/?shop-comment_list" paraments:dic1  addView:self.view addNavgationController:self.navigationController success:^(id responseDic) {
        ...
        if ([ [NSString stringWithFormat:@"%@",[dict objectForKey:@"error"]]isEqualToString:@"0"]) {
            NSMutableArray *array=[NSMutableArray arrayWithArray:[[dict objectForKey:@"data"] objectForKey:@"items"]];
